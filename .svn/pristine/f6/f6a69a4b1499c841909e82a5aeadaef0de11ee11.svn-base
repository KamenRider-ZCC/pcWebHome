<template>
  <div :class="{'inline-area-picker': inline}">
    <el-select class="mb10x" v-model="province" @change="changeProvince" :placeholder="provinceHolder" :disabled="disabled">
      <el-option v-for="item in provinceList" :key="item.id" :label="item.name" :value="item.id+'-'+item.name"></el-option>
    </el-select>
    <br/>
    <el-select class="mb10x" v-model="city" @change="changeCity" :placeholder="cityHolder" :disabled="disabled">
      <el-option v-for="item in cityList" :key="item.id" :label="item.name" :value="item.id+'-'+item.name"></el-option>
    </el-select>
    <br/>
    <el-select class="mb10x" v-model="district" @change="changeDistrict" :placeholder="districtHolder" :disabled="disabled">
